...
pthread_t start_keepalive_daemon(int client_sock, int server_sock) {
    ...
    KeepAliveArgs *keepalive_args = malloc(sizeof(KeepAliveArgs));
    if (!keepalive_args) {
        perror("malloc");
        close(client_sock);
        close(server_sock);
...
void run_server(int port) {
    ...
    while(1){ // Granart server to work after client disconnect, and waitinf for new connection
        ...
        pthread_t keepalive_thread_id = start_keepalive_daemon(client_sock, server_sock);

        process_current_server_inputting(client_sock, server_sock, buffer, NULL);
        
        end_current_connection(client_sock, keepalive_thread_id);
    }
...
void run_unix_server(const char *socket_path) {
    ...
    while(1){ // Granart server to work after client disconnect, and waitinf for new connection
        ...
        process_current_server_inputting(client_sock, server_sock, buffer, socket_path);
